    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="mb-6 flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
          ...
          <Link
            href="/community/new"
            className="inline-flex items-center justify-center px-6 py-3 bg-atps-yellow text-gray-900 font-semibold rounded-lg hover:opacity-90 transition-opacity shadow-lg hover:shadow-xl whitespace-nowrap"
          >
            + Create Post
          </Link>
        </div>
            ...
            <div className="bg-card rounded-xl shadow-md p-6 sticky top-6 border border-border">
              ...
              <div className="mt-8 pt-8 border-t border-border">
                ...
                <div className="flex flex-wrap gap-2">
                  {['safety', 'tips', 'questions', 'community'].map((tag) => (
                    <button
                      key={tag}
                      className="px-3 py-1 bg-yellow-50 dark:bg-yellow-900/30 text-foreground text-sm rounded-full hover:bg-yellow-100 dark:hover:bg-yellow-900/50 transition-colors"
                    >
                      #{tag}
                    </button>
                  ))}
                ...
                {post.is_pinned && (
                  <div className="bg-atps-yellow text-gray-900 px-4 py-2 text-sm font-semibold flex items-center">
                    <svg className="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20">
                      <path d="M5.5 17.5a.5.5 0 01-1 0V.5a.5.5 0 011 0v17zM3 2.5a.5.5 0 01.5-.5h13a.5.5 0 01.5.5v13a.5.5 0 01-.5.5h-13a.5.5 0 01-.5-.5v-13z"/>
                    </svg>
                    Pinned Post
                  </div>
                )}
              ...
              <div className="p-6">
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-blue-600 dark:from-blue-600 dark:to-blue-700 rounded-full flex items-center justify-center text-white font-bold">
                  ...
                  <div className="flex items-center space-x-2">
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                      post.category === 'announcements' ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300' :
                      post.category === 'tips' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300' :
                      post.category === 'questions' ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300' :
                      post.category === 'safety' ? 'bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300' :
                ...
                <Link href={`/community/${post.id}`}>
                  <h2 className="text-2xl font-bold text-foreground dark:text-white mb-3 hover:text-atps-yellow transition-colors cursor-pointer">
                    {post.title}
                  </h2>
                  <p className="text-text-secondary dark:text-white/90 mb-4 line-clamp-3">{post.content}</p>
                </Link>
                ...
                {post.tags && post.tags.length > 0 && (
                  <div className="flex flex-wrap gap-2 mb-4">
                    {post.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-2 py-1 bg-muted dark:bg-muted text-text-secondary dark:text-white/80 text-xs rounded-full"
                      >
                        #{tag}
                      </span>
                    ))}
